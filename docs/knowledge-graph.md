...
What the **target repo** has under `memory/` after `fabric-vibecoding-agents install` runs, and what shape it has as a graph. Reads top-down: the agent enters at `entry.md`, then traverses outward to session bootstrap → workflow → skills → rules.
...
flowchart TD
    ...
    RULES["rules/  (4 nodes)<br/>━━━━━━━━━━<br/>security — credentials · PII · secrets<br/>data-engineering — idempotent MERGE · DQ · contracts<br/>fabric-platform — fabric-vibe · workspaces.json<br/>notebook-authoring — kernel · packages · params"]:::rule
    RULES --> EXTRA
