...
This repository is the source package and installer for Microsoft Fabric agent profiles. It is not the day-to-day Fabric project workspace. Install a profile into a target repo with `cli/install-fabric-agent` (or `pip install fabric-skills-settings`), then run Codex from that target repo root.
