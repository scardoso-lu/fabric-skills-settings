...
from .repository import ApiKeyStore, _set_store, build_key_store_from_env
...
def install_auth_middleware(app) -> bool:
    ...
    Builds an :class:`~server.auth.repository.ApiKeyStore` from the current
    environment, logs how many keys were loaded, and installs the middleware.
    Returns ``True`` when auth was enabled, ``False`` for local single-user dev
    mode (no keys configured). Raises ``RuntimeError`` if keys exist but
    ``FABRIC_MCP_JWT_SECRET`` is unset or too short.
