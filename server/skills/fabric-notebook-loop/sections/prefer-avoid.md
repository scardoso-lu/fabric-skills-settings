...
- Using `fab import` — requires an interactive Windows console; use `fabric-cli notebook deploy deploy` instead
- Using `fab job run` — same console issue; use `fabric-cli notebook deploy run` instead
...
- Guessing at the failing cell when the smoke test reports `System cancelled the Spark session due to statement execution failures` — `fabric-cli notebook deploy monitor` does NOT return the cell-level traceback; see [[skills/fabric-notebook-loop/diagnosing-opaque-failures]]
