...
- Build notebooks with `fabric-vibe notebook build`
- Deploy and run via `fabric-vibe notebook deploy` — NOT `fab import` or `fab job run` (both require an interactive Windows console and fail in Git Bash / non-interactive environments)
- Monitor via `fabric-vibe notebook deploy monitor` or by checking job instance status through the Fabric REST API
- After a successful run, `fabric-vibe notebook deploy run` automatically fetches the notebook definition back from Fabric into `workspace/<topic>/<name>.Notebook/` (synced with Fabric UI)
