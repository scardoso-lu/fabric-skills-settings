...
description: Workspace discovery, switching, and artifact transfer via fabric-vibe workspace {init,switch,transfer}. workspaces.json is the only source of IDs.
kind: content
links:
...
```bash
fabric-vibe workspace switch list           # show all discovered workspaces; marks active
fabric-vibe workspace switch <displayName>  # set active workspace; writes resource IDs to .env
```
...
fabric-vibe workspace transfer --topic    <topic> --to <displayName>
